...
const OrderPage = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    window.open(`https://wa.me/923127079861?text=${encodeURIComponent(text)}`, "_blank");
  };
  ...
  return (
    <div className="min-h-screen pt-24 pb-16">
      <div className="bakery-container">
        ...
        >
          <span className="text-primary font-medium text-sm uppercase tracking-wider">Quick Order</span>
        ...
        >
          ...
          <div>
            <label className="block text-sm font-medium text-foreground mb-1.5">What would you like to order? *</label>
            <textarea
              value={form.order}
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-sm font-medium text-foreground mb-1.5">Delivery / Pickup</label>
              <select
                value={form.delivery}
                onChange={(e) => setForm({ ...form, delivery: e.target.value })}
                className="w-full bg-secondary text-secondary-foreground rounded-lg px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-primary"
              >
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.98 }}
            className="w-full bg-whatsapp text-whatsapp-foreground py-3.5 rounded-lg font-semibold flex items-center justify-center gap-2 text-lg"
          >
            <Send size={20} /> Confirm Order on WhatsApp
          </motion.button>

          <p className="text-center text-muted-foreground text-xs">
            Your order will be sent directly to our WhatsApp. We'll confirm within minutes! 💛
          </p>
        </motion.form>
      </div>
    </div>
